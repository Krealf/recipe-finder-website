...
const RecipeItem = (props) => {
  const { id, image, title, overview, servings, prepMinutes, cookMinutes } =
    props;
  ...
  return (
    <li className={`${styles.recipes__item} ${styles.recipe}`}>
      <img
      ...
      <ul className={`${styles.recipe__cards}`}>
        <li className={`${styles.recipe__card}`}>
          <span className={`${styles.recipe__icon}`}>
            <svg
      ...
      <RouterLink
        className={`${styles.recipe__view}`}
        to={`/recipe-finder-website/recipes/${id}`}
      >
        View Recipe
      </RouterLink>
    </li>
